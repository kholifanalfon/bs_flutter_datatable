...
typedef BsDatatableBuilder<T> = Widget Function(BsDatatableElement el);

typedef BsDatatableOnRowPressed<int, T> = Function(
    int i, List<BsDataCell> cells);

typedef BsDatatableSearchLabel = Widget Function(String label);
...
class _BsDatatableState extends State<BsDatatable>
  ...
  Map<int, TableColumnWidth> _getColumnsWidths(BuildContext context) {
    ...
    _currentColumns.forEach((column) {
      if (column.width != null)
        ...
        _columnsWidths.addAll(
          {_currentColumns.indexOf(column): FixedColumnWidth(300.0)},
        );
    });
  ...
  List<TableRow> _getRows() {
    ...
    for (int i = 0; i < widget.source.countDataPage; i++) {
      ...
      if (widget.striped)
        backgroundColor = i % 2 == 0 ? Colors.transparent : widget.style.color;

      Color hoverColor = widget.style.hoverColor == null
          ? Color(Theme.of(context).colorScheme.primary.value).withOpacity(0.04)
          : widget.style.hoverColor!;

      _tableRows.add(
        TableRow(
          decoration: BoxDecoration(
            color: _isHover[i]! ? hoverColor : backgroundColor,
            border: Border(
  ...
  Widget searchForm({
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Container(
            child: builderLabel != null
                ? builderLabel(widget.language.searchLabel!)
                : null,
          ),
          Container(
