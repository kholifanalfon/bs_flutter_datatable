...
class BsDataColumn extends StatelessWidget {
  ...
  BsDataColumn({
    ...
    this.height = 20.0,
    this.alignment,
    this.padding = const EdgeInsets.only(left: 12.0, right: 12.0, top: 15.0, bottom: 15.0),
    this.textStyle,
    this.border,
    this.orderable = true,
  ...
  Widget orderIcon(BuildContext context) {
    ...
    icon = Stack(
      children: [
        Positioned(
          child: Container(child: Icon(Icons.arrow_drop_up,
        ...
        Positioned(
          child: Container(margin: EdgeInsets.only(top: 8.0), child: Icon(Icons.arrow_drop_down,
            color: (orderState.ordered && orderState.orderType == BsOrderColumn.desc ? Colors.black : Colors.grey).withOpacity(orderable ? 0.8 : 0),
            size: Theme.of(context).textTheme.bodyText1!.fontSize! + 5.0,
          )
        )),
      ],
